...
E_inf = np.zeros(len(cts.T_ext))
Cv_inf = np.zeros(len(cts.T_ext))
corr_first_inf = np.zeros(len(cts.T_ext))
corr_second_inf = np.zeros(len(cts.T_ext))

for i in range(len(cts.T_ext)):
    E_inf[i] = an.energy_onsager(cts.T_ext[i])
    Cv_inf[i] = an.heat_onsager(cts.T_ext[i])
    corr_first_inf[i] = an.corr_onsager(cts.T_ext[i], cts.i[0])
    corr_second_inf[i] = an.corr_onsager(cts.T_ext[i], cts.i[1])
